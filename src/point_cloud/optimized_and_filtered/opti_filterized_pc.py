...
class PointCloudNode(Node):
    def __init__(self, config):
        ...
        # allow runtime overrides via ROS2 parameters
        ...
        # NEW: voxel leaf size (meters)
        default_voxel = self.task_config.get('voxel_size', 0.05)
        ...
        self.voxel_size              = self.get_parameter('voxel_size').value  # meters
    ...
    # -------------------------------------------
    # Function to Create, Filter, and Publish the RGB Point Cloud
    # -------------------------------------------
    def create_pointcloud(self):
        ...
        try:
            ...
            # ––– 8) VOXEL GRID DOWNSAMPLING –––
            if self.voxel_size > 0:
                # compute voxel indices for each point
                vox_idx = np.floor(pts[:, :3] / self.voxel_size).astype(np.int32)
                # pack 3‐ints into a single binary for uniqueness
                key = vox_idx.view(np.dtype((np.void, vox_idx.dtype.itemsize * 3)))
                _, unique_inds = np.unique(key, return_index=True)
                pts = pts[unique_inds]
            # ––– end voxel filter –––

            # 9) Apply rotation & translation
            R = quat2mat([self.Qw, self.Qx, self.Qy, self.Qz])
            pts[:, :3] = pts[:, :3] @ R.T
            pts[:, :3] += np.array([self.pose_x, self.pose_y, self.pose_z],
                                   dtype=np.float32)

            # 10) Publish via create_cloud
            header = Header()
